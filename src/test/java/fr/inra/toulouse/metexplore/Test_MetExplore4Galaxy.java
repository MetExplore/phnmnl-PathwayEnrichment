...
public class Test_MetExplore4Galaxy extends TestCase {
    ...
    protected void createdDummyFile(String inputLine){
        try {
            ...
            dummyFile.close();
            parsedFile = met.extractData(dir + "dummyFile.tsv", -1);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void setMappingComparaison (String bpe){
        ...
        map = met.mapping(bionet,parsedFile,1,4, inchiLayers);
    }

    public void testMappingCHEBIMap () {
        setMappingComparaison("M_tststeroneglc");
        assertEquals(expectedMap.iterator().next().getName(), map.iterator().next().getName());
    }

    public void testMappingLayerPSelect () {
    ...
    public void testMappingLayerFormulaSelect () {
    ...
    public void testMappingInChIMap () {
    ...
    public void testMappingFiltered () {
        try{
            ...
            parsedFile = met.extractData(dir + "dummyFile.tsv", 24);
            setMappingComparaison("M_tststeroneglc");
    ...
    public void testWriteOutput() {
        ...
        try {
            ...
            bo.readLine();
            assertEquals(bo.readLine(), "Steroid metabolism\t0.02\t0.02\t0.02\t0.02\ttestosterone 3-glucosiduronic acid\t1\t1.67");
            fo.delete();
        } catch (IOException e) {
            e.printStackTrace();
        }
