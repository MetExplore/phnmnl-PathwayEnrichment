...
public class Test_MetExplore4Galaxy extends TestCase {
    ...
    protected void createDummyFile(String inputLine, String header){
        try {
            ...
            dummyFile.close();
            met.list_lineInFile = new HashMap<String, String[]>();
            met.extractData();
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public void createDummyFile4Galaxy(String inputLine){
        createDummyFile(inputLine, "\"variableMetadata\\tdatabase_identifier\\tchemical_formula\\tsmiles\\tinchi\\tmetabolite_identification\\tmass_to_charge\\tmass_of_proton\\tmass\\tfragmentation\\tmodifications\\tcharge\\tretention_time\\treliability\\tsample_mean\\tsample_sd\\tsample_CV\\tpool_mean\\tpool_sd\\tpool_CV\\tpoolCV_over_sampleCV\\tchebi.id\\tchemspider.id\\tbiodb.compound.name");
    }

    public void createDummyFileMetExploreID(String inputLine){
       createDummyFile(inputLine, "SBML_ID");
    }

    public void testExtractData() {
        createDummyFile4Galaxy("Testosterone glucuronide\tCHEBI:28835\tC25H36O8\t[H][C@@]12CCC3=CC(=O)CC[C@]3(C)[C@@]1([H])CC[C@]1(C)[C@H](CC[C@@]21[H])O[C@@H]1O[C@@H]([C@@H](O)[C@H](O)[C@H]1O)C(O)=O\tNA\tTestosterone glucuronide\t463,2329\t1,00727647\t464,24017647\tNA\t[(M-H)]-\t1\t7,9\t4\t2,1475578771\t0,5701078279\t0,265467969\t178149,617939526\t12351,5841321731\t0,0693326445\t0,2611714128\t28835\tNA\ttestosterone 17-glucosiduronic acid\n");

        HashMap<String, String[]> obtainedParsing = met.list_lineInFile;
    ...
    public void setMappingComparaison (String bpe) {
        ...
        met.list_mappingBpe=new HashSet<BioPhysicalEntity>();
        try{
            met.mapping();
            map = met.list_mappingBpe;
        } catch (IOException e) {
            e.printStackTrace();
        }
    }

    public void testMappingCHEBI () throws SecurityException {
        createDummyFile4Galaxy("Testosterone glucuronide\tCHEBI:28835\tC25H36O8\t[H][C@@]12CCC3=CC(=O)CC[C@]3(C)[C@@]1([H])CC[C@]1(C)[C@H](CC[C@@]21[H])O[C@@H]1O[C@@H]([C@@H](O)[C@H](O)[C@H]1O)C(O)=O\tNA\tTestosterone glucuronide\t463,2329\t1,00727647\t464,24017647\tNA\t[(M-H)]-\t1\t7,9\t4\t2,1475578771\t0,5701078279\t0,265467969\t178149,617939526\t12351,5841321731\t0,0693326445\t0,2611714128\t28835\tNA\ttestosterone 17-glucosiduronic acid\n");
        setMappingComparaison("M_tststeroneglc");
        assertEquals(expectedMap.iterator().next().getName(), map.iterator().next().getName());
    }

    public void testMappingLayerP () throws SecurityException {
        createDummyFile4Galaxy("Malic acid\tCHEBI:6650\tC4H6O5\tOC(CC(O)=O)C(O)=O\tInChI=1S/C4H6O5/c5-2(4(8)9)1-3(6)7/h2,5H,1H2,(H,6,7)(H,8,9)\tMalic acid\t133,013511\t1,00727647\t134,02078747\tNA\t[(M-H)]-\t1\t1,08\t5\t2,705885808\t0,2514229882\t0,0929170726\t455610,344205852\t48548,3017483153\t0,1065566275\t1,1467927746\t6650\tNA\tmalic acid");
        inchiLayers[3]="p";
    ...
    public void testMappingLayerFormula () throws SecurityException {
        createDummyFile4Galaxy("Cinnamoylglycine\tCHEBI:68616\tC11H11NO3\tOC(=O)CNC(=O)C=Cc1ccccc1\tInChI=1S/C11H11NO3/c13-10(12-8-11(14)15)7-6-9-4-2-1-3-5-9/h1-7H,8H2,(H,12,13)(H,14,15)/b7-6+\tCinnamoylglycine\t204,065452\t1,00727647\t205,07272847\tNA\t[(M-H)]-\t1\t7,03\t5\t4,0160399219\t0,5133270871\t0,1278192192\t11742041,4996239\t2134365,54261586\t0,1817712484\t1,4220963763\t68616\tNA\tN-cinnamoylglycine");
        inchiLayers[0]="";
        inchiLayers[1]="";
        met.inchiLayers=inchiLayers;
    ...
    public void testMappingInChI () throws SecurityException {
        createDummyFile4Galaxy("Taurine\tNA\tC2H7NO3S\tC(CS(O)(=O)=O)N\tInChI=1S/C2H7NO3S/c3-1-2-7(4,5)6/h1-3H2,(H,4,5,6)\tTaurine\t124,006693\t1,00727647\t125,01396947\tNA\t[(M-H)]-\t1\t0,88\t5\t2,6122895216\t0,6358457794\t0,2434055545\t387859,346882448\t11652,3712684191\t0,0300427755\t0,1234268278\t15891\tNA\ttaurine");
        setMappingComparaison("M_taur");
        assertEquals(expectedMap.iterator().next().getName(), map.iterator().next().getName());
    }

    public void testMappingFail () throws SecurityException {
        createDummyFile4Galaxy("Pantothenic acid\tNA\tC9H17NO5\tCC(C)(CO)C(O)C(=O)NCCC(O)=O\tNA\tPantothenic acid\t218,102478\t1,00727647\t219,10975447\tNA\t[(M-H)]-\t1\t4,77\t5\t3,5599610222\t0,2982536819\t0,0837800414\t3012837,77207209\t131428,160471926\t0,043622714\t0,5206814567\t7916\tNA\tpantothenic acid");
        setMappingComparaison("M_pnto_R");

        BioPhysicalEntity bpe = bionet.getBioPhysicalEntityById("M_pnto_R");
        String[] valuesList = met.list_lineInFile.values().iterator().next();
...
//    }
    ...
    public void testMappingMetExploreID () throws SecurityException {
        met.idSBMLColumn=0;
        met.chebiColumn=met.inchiColumn=-1;
        createDummyFileMetExploreID("M_taur");
        setMappingComparaison("M_taur");
        System.out.println(expectedMap.iterator().next().getName());
        System.out.println(map.iterator().next().getName());
        assertEquals(expectedMap.iterator().next().getName(), map.iterator().next().getName());
    }
}